  ...
  return (
    <div className="fixed inset-0 z-[5000] flex items-center justify-center p-2 sm:p-4 animate-fade-in">
      ...
      <div className="relative w-full max-w-[96%] sm:max-w-[95%] lg:max-w-[90%] bg-surface-card rounded-[1.5rem] md:rounded-[2.5rem] border border-[var(--surface-border)] shadow-2xl flex flex-col max-h-[92vh] md:max-h-[85vh] overflow-hidden animate-in fade-in zoom-in duration-400">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-8 space-y-4 md:space-y-6 min-h-0 bg-surface-card/30">
          ...
          {activeTab === 'DEPORTIVO' && (
            <div className="space-y-4 md:space-y-5 animate-in slide-in-from-bottom-4 fade-in duration-500">
              {matchedAssignment && (
                <div className="p-3.5 bg-primary-500/10 border border-primary-500/20 rounded-2xl flex items-center gap-3">
              ...
              <div className="pt-1 flex justify-end">
                <button
                  ...
                  className="flex items-center justify-center gap-2 bg-primary-500 hover:bg-primary-600 active:scale-95 text-white px-5 py-2.5 rounded-xl font-black uppercase tracking-wider text-[9px] sm:text-[10px] shadow-lg shadow-primary-500/10 hover:scale-[1.02] transition-all disabled:opacity-50"
                >
                  {isSavingSports ? (
          ...
          {activeTab === 'CONTACTO' && (
            <div className="space-y-4 md:space-y-5 animate-in slide-in-from-bottom-4 fade-in duration-500">
              <div className="flex justify-between items-center bg-surface-ground p-3.5 rounded-2xl border border-[var(--surface-border)]">
                <div>
                  <h4 className="text-[10px] md:text-[11px] font-black uppercase tracking-widest text-[var(--text-main)]">Contactos de Emergencia</h4>
                  <p className="text-[9px] text-[var(--text-muted)] mt-0.5 font-bold">Máximo 3 contactos por jugador</p>
                </div>
                {contactsList.length < 3 && !isAddingContact && (
                  <button
                    type="button"
                    onClick={handleAddContactClick}
                    className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-[9.5px] font-black uppercase tracking-wider transition-all cursor-pointer inline-flex whitespace-nowrap leading-none shrink-0"
                  >
                    <PlusCircle size={11} />
                    Agregar Contacto
                  </button>
              ...
              {contactsSaveSuccess && (
                <div className="p-3 bg-emerald-500/10 border border-emerald-500/30 text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase rounded-xl tracking-widest text-center animate-in fade-in duration-300">
                  ✓ ¡CONTACTOS ACTUALIZADOS CORRECTAMENTE!
                </div>
              )}

              {isAddingContact && (
                <div className="p-4 bg-surface-ground rounded-2xl border border-emerald-500/20 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    ...
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-wider pl-1 font-extrabold text-[var(--text-muted)]">Email</label>
                      <input
                    ...
                    <div className="space-y-1 col-span-1 sm:col-span-2 md:col-span-3 lg:col-span-4">
                      <label className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-wider pl-1">Dirección </label>
                      <input
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {contactsList.length === 0 ? (
                  <div className="col-span-full py-10 bg-surface-ground rounded-2xl border border-dashed border-[var(--surface-border)] text-center text-xs text-[var(--text-muted)]">
                  ...
                  contactsList.map((contact, index) => (
                    <div 
                      ...
                      className="p-4 bg-surface-ground rounded-2xl border border-[var(--surface-border)] relative flex flex-col justify-between hover:border-emerald-500/20 transition-all duration-300"
                    >
                      <div>
                        <div className="flex justify-between items-start gap-2 mb-3">
                          <div className="min-w-0">
                            ...
                            <h5 className="font-black text-xs text-[var(--text-main)] mt-1.5 break-words uppercase truncate">{contact.name}</h5>
                          </div>
                        ...
                        <div className="space-y-2 text-[11px] font-bold text-[var(--text-muted)] pt-1 border-t border-[var(--surface-border)]/50">
                          ...
                          <div className="flex items-start gap-1.5 break-all">
                            ...
                            <span className="leading-tight truncate">{contact.address}</span>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <div className="space-y-4 md:space-y-5 animate-in slide-in-from-bottom-4 fade-in duration-500">
              ...
              <div className="pt-1 flex justify-end">
                <button
                  ...
                  className="flex items-center justify-center gap-2 bg-indigo-500 hover:bg-indigo-600 active:scale-95 text-white px-5 py-2.5 rounded-xl font-black uppercase tracking-wider text-[9px] sm:text-[10px] shadow-lg shadow-indigo-500/10 hover:scale-[1.02] transition-all disabled:opacity-50"
                >
                  {isSavingSchooling ? (
                    <Loader2 className="animate-spin" size={12} />
                  ) : (
        ...
        {/* Footer (Fijo / No scrollable) */}
        <div className="shrink-0 px-6 sm:px-8 py-4 bg-surface-ground border-t border-[var(--surface-border)] flex justify-end">
           <button 
            onClick={onClose}
            className="px-6 py-2.5 bg-[var(--text-main)] text-surface-card rounded-xl font-black uppercase italic tracking-widest text-[9.5px] hover:translate-y-[-1px] transition-all"
          >
            Cerrar Legajo
          </button>
        </div>
...
const InfoCard = ({ icon: Icon, label, value }: { icon: any, label: string, value: string }) => (
  <div className="p-3.5 rounded-2xl bg-surface-ground border border-[var(--surface-border)] flex items-center gap-3 hover:border-primary-500/30 transition-all group">
    <div className="w-9 h-9 rounded-lg bg-surface-card flex items-center justify-center text-[var(--text-muted)] group-hover:text-primary-500 transition-colors shadow-sm shrink-0">
      <Icon size={16} />
    </div>
    <div className="min-w-0">
      <p className="text-[8px] font-black text-[var(--text-muted)] uppercase tracking-widest truncate leading-none mb-1">{label}</p>
      <p className="text-xs md:text-[13px] font-black text-[var(--text-main)] italic truncate leading-snug">{value || 'No asignado'}</p>
    </div>
  </div>
);
