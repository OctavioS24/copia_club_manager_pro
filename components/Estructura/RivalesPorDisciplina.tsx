      ...
      <div className="bg-white dark:bg-[#0f1219] rounded-[3.5rem] border border-slate-200 dark:border-white/5 p-12 shadow-sm">
        <div className="flex items-center justify-between mb-10">
          <h3 className="text-2xl font-black uppercase tracking-tighter dark:text-white italic flex items-center gap-3">
            ...
            Rivales para {selectedDiscipline}
          </h3>
          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
            {rivals.length} Rivales Registrados
          </span>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {rivals.length === 0 ? (
              ...
              rivals.map((rival) => (
                <div key={rival.id} className="group flex items-center gap-4 bg-slate-50 dark:bg-white/5 p-4 rounded-2xl border border-transparent hover:border-pink-500/30 transition-all">
                  <div className="flex-1">
                    {editingId === rival.id ? (
                      <input 
                        autoFocus
