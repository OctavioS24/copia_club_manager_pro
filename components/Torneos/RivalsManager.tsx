    ...
    <AnimatePresence>
      ...
      >
        <motion.div 
          ...
          className="bg-white dark:bg-[#0f121a] w-full max-w-xl rounded-[3rem] shadow-2xl border border-slate-200 dark:border-white/5 overflow-hidden flex flex-col"
        >
          {/* Header */}
          ...
          <div className="p-10 space-y-8 flex-1 overflow-y-auto max-h-[60vh] custom-scrollbar">
            ...
            <form onSubmit={handleAddRival} className="flex gap-4">
              <input 
                ...
                className="flex-1 px-6 py-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl font-bold text-sm outline-none border border-transparent dark:border-white/5 focus:border-pink-600 shadow-inner dark:text-white transition-all"
              />
              <button 
                type="submit"
                disabled={isSubmitting || !newRivalName.trim()}
                className="bg-pink-600 text-white px-6 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl shadow-pink-600/20 hover:scale-105 active:scale-95 transition-all flex items-center gap-2 disabled:opacity-50 disabled:hover:scale-100"
              >
            ...
            <div className="space-y-3">
              {isLoading ? (
                ...
                rivals.map(rival => (
                  <div 
                    key={rival.id}
                    className="flex items-center justify-between p-5 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-100 dark:border-white/5 group hover:border-pink-600/30 transition-all"
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-slate-200 dark:bg-slate-800 flex items-center justify-center">
                        <Shield size={18} className="text-slate-400 group-hover:text-pink-600 transition-colors" />
                      </div>
